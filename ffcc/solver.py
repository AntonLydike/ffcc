...
class SMTSolver:
    ...
    def _kind(self, name):
        ...
        return ('float'
                if self._expr(name).sort_kind() == Z3_FLOATING_POINT_SORT
                else 'int')
...
class SMTSynthesizer:
    ...
    def find_conflict(self, tunable, delta):
        checkpoints = np.random.uniform(low=self.low, high=self.high, size=self.sample_num).astype(np.float32)
        if self.prog.name == "qsqrt":
    ...
    def sat(self, io_pairs, delta):
        ...
        for idx, (inp, out) in enumerate(io_pairs):
            ...
            solver.add_output_constraint('r_%d'%idx, out, delta=delta * (out + self.epsilon))
        res = solver.solve()
        if res == None:
            return None
        tunable = []
