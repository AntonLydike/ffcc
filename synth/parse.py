...
def parse_ssa(text: str, baselineno: int = 0) -> IRNode:
    ...
    for lineno, line in enumerate(text.splitlines(), start=baselineno):
        ...
        pos, valname = val
        if valname[0] != '%':
            raise ParseError(lineno, line, "Expected SSA result value here (%name)", pos)
        pos, eq = eq
        if eq != '=':
        ...
        match parts:
            case [(_, 'constant'), cst, (_, ':'), typ]:
                op = ConstantNode(_parse_constant(lineno, line, *cst), _parse_type(lineno, line, *typ))
            case [(_, 'var'), name, (_, ':'), typ]:
                op = VarNode(_parse_str_lit(lineno, line, *name), _parse_type(lineno, line, *typ))
            ...
            case [(_, 'bitcast'), direction, arg, (_, 'to'), dst]:
                ...
                if dir not in ('i2f', 'f2i'):
                    raise ParseError(lineno, line, 'Expected either f2i or i2f direction', pos)
                op = BitCastOperator(
                ...
                if op.type != res_t:
                    raise ParseError(lineno, line, f'Result type mismatch, expected {op.type}, got {res_t}', pos)
            ...
            case [(pos, kind), *args, (_, ':'), typ]:
                if len(args) == 2:
                    raise ParseError(lineno, line, f'Unknown operation format for {kind}: {args} (missing comma?)', pos)
                raise ParseError(lineno, line, f'Unknown operation format for {kind}', pos)
            case [(pos, kind), *args, (_, 'to'), typ]:
                raise ParseError(lineno, line, f'Unknown operation format for cast op {kind}: {args}', pos)
            case [(pos, kind), *args]:
                raise ParseError(lineno, line, f'Unknown missing result type annotation for {kind}: {args}', pos)
        if op is None:
...
def _parse_val(lineno: int, line: str, pos: int, val: str, vals: dict[str, Value]) -> Value:
    if '%' != val[0]:
        raise ParseError(lineno, line, 'Expected SSA Value (%name)', pos)
    if val not in vals:
        raise ParseError(lineno, line, f'Value use before definition (falue name {repr(val)})', pos)
    return vals[val]
...
def _parse_type(lineno: int, line: str, pos: int, type: str) -> Type:
    ...
    raise ParseError(lineno, line, f"Malformed type, expected to start with f or i, got {repr(type)}", pos)
...
def _parse_str_lit(lineno: int, line: str, pos: int, text: str) -> str:
    ...
    try:
        ...
        raise ParseError(lineno, line, f"Malformed string literal {repr(text)}: {ex}", pos) from ex
    except (MemoryError, RecursionError) as ex:
        raise ParseError(lineno, line, f"Serious error in string parsing: {ex}", pos) from ex
